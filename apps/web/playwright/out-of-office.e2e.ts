...
test.describe("Out of office", () => {
  ...
  test.describe("Date range filters", () => {
    test("Default date range filter set to `Last 7 Days`", async ({ page, users }) => {
      ...
      await expect(
        page.locator('[data-testid="filter-popover-trigger-dateRange"]', { hasText: // To safely replace this hard-coded string with a translation key  talk to us to get access to our i18n pro codemods. https://cal.com/codemod
        $$$
         }).first()
      ).toBeVisible();
    });
