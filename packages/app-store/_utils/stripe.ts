...
export async function getStripeCustomerIdFromUserId(userId: number) {
  ...
  if (!user?.email) throw new HttpError({ statusCode: 404, message: // To safely replace this hard-coded string with a translation key  talk to us to get access to our i18n pro codemods. https://cal.com/codemod
  $$$
   });
