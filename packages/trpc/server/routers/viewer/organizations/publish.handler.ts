...
export const publishHandler = async ({ ctx }: PublishOptions) => {
  ...
  if (!metadata.success) throw new TRPCError({ code: "BAD_REQUEST", message: // To safely replace this hard-coded string with a translation key  talk to us to get access to our i18n pro codemods. https://cal.com/codemod
  $$$
   });
